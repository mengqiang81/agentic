...
## 更新摘要
**变更内容**
- 更新了 AgentEventMapper 中文本增量缓冲系统的性能优化部分
- 新增了缓冲阈值设置和 flush 机制的技术细节
- 补充了事件处理优化的相关说明
- 更新了性能考虑章节中的缓冲系统相关内容
...
- AgentEventMapper：将 AgentScope 事件转换为 AG-UI 事件，过滤掉不对外暴露的内部事件。**新增**：引入 per-run 有状态的 RunMapper，包含文本增量缓冲系统和工具调用事件缓冲机制。
...
- **新增**：引入 per-run 有状态的 RunMapper，提供完整的事件缓冲和合并机制。
- **文本增量缓冲系统**：通过 StringBuilder 和阈值控制减少前端事件数量，优化 AG-UI 开发模式下的性能。
- **工具调用事件缓冲**：缓存 ToolCallStart/Delta/End 事件，等待 ToolResultTextDelta 或 ToolResultEnd 确认工具正常执行后才 flush。
- **中断处理缓冲**：当 RequireUserConfirmEvent 到达时，丢弃被中断工具的缓冲事件，仅通过 CUSTOM 事件传递中断信息。
...
class RunMapper {
+map(event) AgUiEvent[]
+flushTextDeltaBuffer() AgUiEvent[]
+flushToolCallBuffer(toolCallId) AgUiEvent[]
}
class AgUiEvent {
+getType() String
+getData() String
...
RunMapper --> AgUiEvent : "创建"
```

**图表来源**
- [AgentEventMapper.java:55-57](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L55-L57)
- [AgentEventMapper.java:71-92](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L71-L92)
...
### 文本增量缓冲系统优化
**新增**：AgentEventMapper 引入了高性能的文本增量缓冲系统，专门针对 AG-UI 开发模式下的性能优化。

- **缓冲阈值设置**：TEXT_BUFFER_FLUSH_THRESHOLD 设为 20，平衡事件粒度与性能开销。
- **flush 机制**：当缓冲区长度达到阈值时自动 flush，或在 TextBlockEndEvent 时强制 flush 剩余内容。
- **性能优化**：减少前端事件数量，避免 AG-UI 开发模式下 structuredClone + Object.freeze 的 O(n²) 性能退化。
- **内存管理**：使用 StringBuilder 累积文本增量，setLength(0) 重置缓冲区，避免内存泄漏。

```mermaid
flowchart TD
A["TextBlockDeltaEvent 到达"] --> B["追加到 textDeltaBuffer"]
B --> C{"缓冲区长度 >= 20?"}
C --> |是| D["flushTextDeltaBuffer()"]
C --> |否| E["等待更多增量"]
D --> F["创建 TEXT_MESSAGE_CONTENT 事件"]
F --> G["清空缓冲区"]
E --> H["继续累积"]
```

**图表来源**
- [AgentEventMapper.java:144-179](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L144-L179)

**章节来源**
- [AgentEventMapper.java:84-87](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L84-L87)
- [AgentEventMapper.java:144-179](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L144-L179)

### 工具调用事件缓冲机制
**新增**：AgentEventMapper 实现了复杂的工具调用事件缓冲机制，确保事件顺序的正确性和前端体验的一致性。

- **事件缓冲策略**：ToolCallStart → ToolCallDelta → ToolCallEnd 事件在缓冲区中累积，直到确认工具正常执行。
- **正常执行流程**：收到 ToolResultTextDeltaEvent 或 ToolResultEndEvent 时，flush 缓冲的 START/ARGS/END 事件。
- **中断处理**：收到 RequireUserConfirmEvent 时，丢弃被中断工具的缓冲事件，仅通过 CUSTOM 事件传递中断信息。
- **消息 ID 管理**：为工具结果事件分配唯一的 messageId（"result-" + toolCallId），确保前端正确关联。

```mermaid
flowchart TD
A["ToolCallStartEvent"] --> B["缓冲 TOOL_CALL_START"]
B --> C["ToolCallDeltaEvent"]
C --> D["缓冲 TOOL_CALL_ARGS"]
D --> E["ToolCallEndEvent"]
E --> F["缓冲 TOOL_CALL_END"]
F --> G{"收到 ToolResultTextDelta?"}
G --> |是| H["flush 缓冲事件"]
G --> |否| I{"收到 RequireUserConfirm?"}
I --> |是| J["丢弃缓冲事件"]
I --> |否| K["等待更多事件"]
H --> L["创建 TOOL_CALL_RESULT"]
J --> M["创建 CUSTOM 中断事件"]
```

**图表来源**
- [AgentEventMapper.java:197-293](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L197-L293)

**章节来源**
- [AgentEventMapper.java:63-69](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L63-L69)
- [AgentEventMapper.java:197-293](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L197-L293)
...
- **新增**：支持多种 HITL 恢复方式，包括 AG-UI 标准 resume 字段、CopilotKit forwardedProps.command 格式和旧版 confirm_results 字段。
...
- **新增**：文本增量缓冲系统优化前端性能，通过阈值控制减少事件数量，避免 AG-UI 开发模式下的性能退化。
- **新增**：工具调用事件缓冲机制确保事件顺序正确，避免前端看到"工具正在执行→再弹确认框"的错误状态。
...
- **新增**：文本增量缓冲问题
  - 检查 TEXT_BUFFER_FLUSH_THRESHOLD 阈值设置是否合理。
  - 确认 flushTextDeltaBuffer() 方法是否正确调用。
  - 验证 TextBlockEndEvent 是否正确触发剩余缓冲的 flush。
- **新增**：工具调用事件缓冲问题
  - 确认 ToolResultTextDeltaEvent 是否正确触发工具调用事件的 flush。
  - 检查 RequireUserConfirmEvent 是否正确丢弃被中断工具的缓冲事件。
  - 验证工具调用 ID 的匹配和消息 ID 的生成。
- 多租户串台
...
## 结论
代理管理模块通过明确的多租户隔离策略、严谨的事件映射与过滤、以及生产级的 HarnessAgent 配置，实现了稳定、可观测、可扩展的 AG-UI 协议 SSE 事件流服务。其响应式架构与分布式存储、沙箱隔离、上下文压缩等特性共同保障了高并发场景下的性能与可靠性。**新增的 AgentEventMapper 中文本增量缓冲系统和工具调用事件缓冲机制进一步提升了系统的性能和用户体验，通过阈值控制和智能 flush 机制优化了前端渲染性能，确保事件顺序的正确性和中断处理的可靠性。**
...
- **新增**：合理配置文本增量缓冲阈值，平衡事件粒度与性能开销。
- **新增**：理解工具调用事件缓冲机制，确保中断处理的正确性。
