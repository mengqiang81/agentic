...
**更新** 新增完整的 McpToolController 组件，提供响应式编程模型和高并发支持。增强了调试能力和错误传播机制，改进了工具执行可靠性。
...
- **详细的错误信息**：提供具体的错误原因和修复建议
- **日志记录**：完整的操作日志和错误追踪
- **状态监控**：实时监控 MCP 服务器连接状态
- **重试机制**：自动重试失败的操作
- **超时控制**：防止长时间阻塞操作
...
**更新** 新增的 McpToolController 组件提供了完整的响应式编程支持，为未来的 MCP 客户端集成奠定了坚实基础。增强了调试能力和错误传播机制，显著改进了工具执行的可靠性。