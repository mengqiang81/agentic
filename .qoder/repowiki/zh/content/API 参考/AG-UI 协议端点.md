...
## 更新摘要
**所做更改**
- 新增完整的 AG-UI 协议实现文档，包括控制器、SSE 事件流处理和事件映射机制
- 更新多租户上下文处理，支持 X-Tenant-Id、X-User-Id、X-Project-Id 头部参数
- 完善 SSE 事件流处理机制，包括背压处理和调度器配置
- 新增完整的事件映射规则和 AG-UI 事件类型定义
- 更新性能优化建议，包括 Redis 分布式存储和上下文压缩配置
...
## 简介
本文件为 AG-UI 协议端点的详细 API 文档，聚焦于 POST /awp/v1/runs 端点的实现与使用。该端点采用 HTTP SSE（Server-Sent Events）进行流式响应，支持多租户隔离（通过 X-Tenant-Id、X-User-Id、X-Project-Id 头部参数）。文档涵盖：
...
- 事件映射：AgentEventMapper 将 Agent 事件映射为 AG-UI 事件
...
- 事件映射 AgentEventMapper
  - 将 Agent 事件类型映射为 AG-UI 事件类型，并序列化为 JSON 字符串
...
Controller->>Session : "streamAgentEvents(tenantId, userId, threadId, userMessage)"
...
Session->>Mapper : "toAgUiEvent(event)"
Mapper-->>Session : "AgUiEvent(JSON 字符串)"
Session-->>SSE : "ServerSentEvent(type, data)"
SSE-->>Client : "SSE 事件流"
```
...
- 事件流处理
  ...
  - 使用 boundedElastic 调度器处理阻塞操作
  - 映射事件：agentEventMapper.toAgUiEvent(event)
  - 过滤未映射事件：filter(非空)
  - 转换为 SSE：ServerSentEvent.builder().event(type).data(data).build()
...
- [AgentEventMapper.java:45-104](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L45-L104)
- [AgUiEvent.java:6-23](file://src/main/java/com/example/agentic/agent/AgUiEvent.java#L6-L23)
...
### 事件映射：AgentEventMapper
- 映射规则（依据 agentscope-core 2.0.0-RC3 实际事件类型）
...
- 未映射事件（如 ModelCallStart/End、ToolResultStart、ThinkingBlock 等）作为内部事件忽略
- 序列化：直接返回 JSON 字符串（AgUiEvent.data）
- 安全处理：对 null 值进行安全处理，避免空指针异常

**章节来源**
- [AgentEventMapper.java:15-97](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L15-L97)
...
**章节来源**
- [AgentSessionService.java:57-58](file://src/main/java/com/example/agentic/agent/AgentSessionService.java#L57-L58)
...
## 结论
POST /awp/v1/runs 端点通过响应式 SSE 提供 AG-UI 协议的事件流，结合多租户上下文与事件映射机制，实现了稳定、可扩展的智能体运行接口。配合 Redis 分布式存储、上下文压缩和 OTEL 追踪，满足生产级部署需求。系统支持完整的 AG-UI 事件协议，包括运行开始、消息内容、工具调用等事件类型，为前端提供了丰富的交互能力。
...
- 请求体（RunAgentInput）
  - 示例字段：
    ...
    - state: 会话状态（可选）
- 响应（SSE 事件）
  - 事件类型（event）与数据（data）由 AgentEventMapper 映射生成
  - 事件类型包括：RUN_STARTED、TEXT_MESSAGE_CONTENT、TEXT_MESSAGE_END、TOOL_CALL_START、TOOL_CALL_END、TOOL_CALL_RESULT、RUN_FINISHED
- 完整请求/响应示例（路径参考）
  ...
  - 事件映射规则：[AgentEventMapper.java:18-28](file://src/main/java/com/example/agentic/agent/AgentEventMapper.java#L18-L28)
  - SSE 事件构造：[AgentSessionService.java:62-65](file://src/main/java/com/example/agentic/agent/AgentSessionService.java#L62-L65)
...
- AG-UI 事件类型说明：
  - RUN_STARTED：运行开始事件，包含 session_id 和 reply_id
  - TEXT_MESSAGE_CONTENT：文本消息增量内容，包含 delta、reply_id 和 block_id
  - TEXT_MESSAGE_END：文本消息结束事件，包含 reply_id
  - TOOL_CALL_START：工具调用开始事件，包含 tool_call_id 和 tool_name
  - TOOL_CALL_END：工具调用结束事件，包含 tool_call_id 和 tool_name
  - TOOL_CALL_RESULT：工具调用结果事件，包含 tool_call_id、tool_name 和 state
  - RUN_FINISHED：运行结束事件，包含 reply_id
